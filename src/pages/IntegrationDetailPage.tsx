...
export default function IntegrationDetailPage() {
  ...
  useEffect(() => {
    ...
    workflowRepository.getWorkflows({ integration: slug }, 1, 100)
      .then(res => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to="/integrations" className="inline-flex items-center gap-1.5 text-xs text-zinc-500 hover:text-violet-500 transition-colors">
      ...
      <div className="flex items-center gap-3 border-b border-zinc-200 dark:border-zinc-800 pb-5">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 dark:text-white">{name} Integration</h1>
          <p className="text-zinc-500 dark:text-zinc-400 text-sm">Found {workflows.length} workflows using this service.</p>
        </div>
      </div>
      ...
      {workflows.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-900/5">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">No workflows found using this integration.</p>
        </div>
      ) : (
